...
class CastilloGameState(object):
  ...
  def __init__(self, game, players, rewards, board, grandes, king):
    self._game = game
    self._cur_player = 0
    self._num_players = math.min(_MAX_PLAYERS,players)
  ...
  def _score_one_region(self,region):
    ...
    for idx in range(len(cab_counts)):
      ...
      if cp>0:
        ranks[idx]=1
        anchor_rank=-1
        for idx2 in range(len(cab_counts)):
          if idx!=idx2:
    ...
    for k,rank in ranks:
      if rank>0 and rank<=3:
        final_scores[rank]=self._rewards[region][rank-1]
      if rank==1:
        if self._grandes[k]==region:
          final_scores[rank]=final_scores[rank]+2
  ...
  def apply_action(self, action):
    ...
    self._board[self._cur_player] = _ZERO_CHAR
    self._board[region_idx]=chr(ord(self._board[region_idx])+castillo_count)
    self._history.append(action)
    
    if self._all_moved():
      final_scores = self._score_all_regions()
      min_score = min(final_scores)
      self._win_points = [f-min_score for f in final_scores]
